...
function render(module, scope, key, hints) {
   ...
   for (var i = 0; i < els.length; i++) {
      ...
      if (els[i].astemplate) val = renderTemplate(els[i], val, scope, hints)
...
function renderTemplate(el, val, scope, hints) {
   var range = [0, val.length-1]
   if (el.getAttribute('aslist') && el.getAttribute('aslist').match(/^[0-9]+:[0-9]+$/)) {
   ...
   if (el.astemplate.match(/^func:/)) {
      ...
      if (!el.getAttribute('aslist')) {
         ...
         var result = ''
         if (hints && !el.tagName.match(/^(input|textarea)$/i)) {
            el.asTplArgs[el.asValIndex] = val[0]
            var str = el.asTplFunc.apply(scope, el.asTplArgs)
            var b = document.createElement('div')
            b.style.display = 'none'
            b.innerHTML = str
            var count = b.children.length
            
            if (el.children.length % count == 0) {
               var len = val.length
               for (var i = 0; i < hints.remove.length; i++) {
                  var index = hints.remove[i]
                  if (index < range[0] || index > range[1]) continue
                  for (var j = 0; j < count; j++) {
                     el.removeChild(el.children[index-range[0]])
                     len--
                  }
               }
               for (var i = 0; i < hints.add.length; i++) {
                  var index = hints.add[i]
                  if (index < range[0] || index > range[1]) continue
                  el.asTplArgs[el.asValIndex] = val[index]
                  str = el.asTplFunc.apply(scope, el.asTplArgs)
                  var b = document.createElement('div')
                  b.style.display = 'none'
                  b.innerHTML = str
                  if (index < len) {
                     var ref = el.children[count * (index-range[0])]
                  }
                  for (var j = 0; j < count; j++) {
                     if (index < len && ref) {
                        el.insertBefore(b.children[j], ref)
                     }
